...
if($saction=='save6'){
 ...
 if($saction6=='SaveOption1'){
   ...
   if($_POST['save_to_group']==1){
      if($_POST['the_group_id']>0){  
	    ...
	    foreach($config_ids as $config_id){  //echo $config_id->id.'<br/>'; 
          ...
          for($i=0,$max=count($para1);$i<$max;$i++){
            ...
            $wpdb->query($wpdb->prepare("insert into $t_ap_config_option(config_id,option_type,para1,para2) values (%d,%d,%s,%s)",$config_id->id,1,trim($para1[$i]),trim($para2[$i])));
          }
		}
        $msg = '<div class="updated fade"><p>'.__('Updated To Group!','wp-autopost').'</p></div>';  
      }else{
      ...
      for($i=0,$max=count($para1);$i<$max;$i++){
        if(trim($para1[$i])==''||trim($para1[$i])==null)continue;
        $wpdb->query($wpdb->prepare("insert into $t_ap_config_option(config_id,option_type,para1,para2) values (%d,%d,%s,%s)",$_POST['id'],1,trim($para1[$i]),trim($para2[$i])));
      }
	  $msg = '<div class="updated fade"><p>'.__('Updated!','wp-autopost').'</p></div>';
    }
  ...
  if($saction6=='SaveOption5'){
	...
	if($_POST['save_to_group']==1){
      if($_POST['the_group_id']>0){  
	    ...
	    foreach($config_ids as $config_id){  //echo $config_id->id.'<br/>'; 
          ...
          for($i=0,$max=count($para1);$i<$max;$i++){
            ...
            $wpdb->query($wpdb->prepare("insert into $t_ap_config_option(config_id,option_type,para1,para2) values (%d,%d,%s,%s)",$config_id->id,5,trim($para1[$i]),trim($para2[$i]) ));
          }
      ...
      for($i=0,$max=count($para1);$i<$max;$i++){
       if(trim($para1[$i])==''||trim($para1[$i])==null)continue;
	   if(trim($para2[$i])==''||trim($para2[$i])==null){ $para2[$i] = 0; }
       $wpdb->query($wpdb->prepare("insert into $t_ap_config_option(config_id,option_type,para1,para2) values (%d,%d,%s,%s)",$_POST['id'],5,trim($para1[$i]),trim($para2[$i]) ));
      }   
	  $msg = '<div class="updated fade"><p>'.__('Updated!','wp-autopost').'</p></div>';
    }
    ...
    if($_POST['save_to_group']==1){
      if($_POST['the_group_id']>0){  
	    ...
	    foreach($config_ids as $config_id){  //echo $config_id->id.'<br/>'; 
          ...
          for($i=0,$max=count($para1);$i<$max;$i++){
            if(trim($para1[$i])==''||trim($para1[$i])==null)continue;
            $wpdb->query($wpdb->prepare("insert into $t_ap_config_option(config_id,option_type,para1,para2) values (%d,%d,%s,%s)" ,$config_id->id,2,trim(strtolower($para1[$i])),trim($para2[$i]) ));
          }
		}
        $msg = '<div class="updated fade"><p>'.__('Updated To Group!','wp-autopost').'</p></div>';    
      }else{
      ...
      for($i=0,$max=count($para1);$i<$max;$i++){
        if(trim($para1[$i])==''||trim($para1[$i])==null)continue;
        $wpdb->query($wpdb->prepare("insert into $t_ap_config_option(config_id,option_type,para1,para2) values (%d,%d,%s,%s)" ,$_POST['id'],2,trim(strtolower($para1[$i])),trim($para2[$i]) ));
      }
	  $msg = '<div class="updated fade"><p>'.__('Updated!','wp-autopost').'</p></div>';
    }
  echo $msg;
    ...
    if($_POST['save_to_group']==1){
      if($_POST['the_group_id']>0){  
	    ...
	    foreach($config_ids as $config_id){  //echo $config_id->id.'<br/>'; 
          ...
          for($i=0,$max=count($para1);$i<$max;$i++){
            if(trim($para1[$i])==''||trim($para1[$i])==null)continue;  
            $wpdb->query($wpdb->prepare("insert into $t_ap_config_option(config_id,option_type,para1,para2,options) values (%d,%d,%s,%s,%s)" ,$config_id->id,3,trim($para1[$i]),trim($para2[$i]),trim($options[$i]) ));
          }
		}
        $msg = '<div class="updated fade"><p>'.__('Updated To Group!','wp-autopost').'</p></div>';    
      }else{
      ...
      for($i=0,$max=count($para1);$i<$max;$i++){
        if(trim($para1[$i])==''||trim($para1[$i])==null)continue;  
        $wpdb->query($wpdb->prepare("insert into $t_ap_config_option(config_id,option_type,para1,para2,options) values (%d,%d,%s,%s,%s)" ,$_POST['id'],3,trim($para1[$i]),trim($para2[$i]),trim($options[$i]) ));
      }
	  $msg = '<div class="updated fade"><p>'.__('Updated!','wp-autopost').'</p></div>';
    }
	...
	if($_POST['save_to_group']==1){
      if($_POST['the_group_id']>0){  
	    ...
	    foreach($config_ids as $config_id){  //echo $config_id->id.'<br/>'; 
          ...
          for($i=0,$max=count($para1);$i<$max;$i++){
            if(trim($para1[$i])==''||trim($para1[$i])==null)continue;
            $wpdb->query($wpdb->prepare("insert into $t_ap_config_option(config_id,option_type,para1,para2) values (%d,%d,%s,%s)", $config_id->id,4,trim($para1[$i]),trim($para2[$i]) ));
          }
		}
        $msg = '<div class="updated fade"><p>'.__('Updated To Group!','wp-autopost').'</p></div>';    
      }else{
      ...
      for($i=0,$max=count($para1);$i<$max;$i++){
        if(trim($para1[$i])==''||trim($para1[$i])==null)continue;
        $wpdb->query($wpdb->prepare("insert into $t_ap_config_option(config_id,option_type,para1,para2) values (%d,%d,%s,%s)", $_POST['id'],4,trim($para1[$i]),trim($para2[$i]) ));
      }
	  $msg = '<div class="updated fade"><p>'.__('Updated!','wp-autopost').'</p></div>';
    }
